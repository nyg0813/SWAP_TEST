...
//				print_bad_pte(vma, addr, ptent, page);
			...
			if (unlikely(page_mapcount(page) < 0))
			{
				if(!(POS_AREA_START <= addr && addr < POS_AREA_END))
					print_bad_pte(vma, addr, ptent, page);
				//else
				//	printk("[POSDEBUG]%s pfn: %lu map_count: %d\n",__func__,page_to_pfn(page),atomic_read(&page->_mapcount));			
			}
		...
		unsigned int flags)
{
	...
	if(POS_AREA_START <= address && address < POS_AREA_END){
		...
		if(pfn == POS_EMPTY){
			...
			//printk("[POSDEBUG] Case : POSEMPTY\n\t\tfunc: %s\tpfn: %ld\tmap_count: %d\taddr: %p\n",__func__,pfn,atomic_read(&page->_mapcount),page);
			exclusive =1;
		}
		else{
			page = pfn_to_page(pfn);
			//printk("[POSDEBUG] Case : NONEMPTY\n\t\tfunc: %s\tpfn: %ld\tmap_count: %d\taddr: %p\n",__func__,pfn,atomic_read(&page->_mapcount),page);
		}
		//printk("[POSDEBUG]\t%s\tpfn\t%ld\tmap_count\t%d\n",__func__,page_to_pfn(page),atomic_read(&page->_mapcount));
	}
	else{
		page = alloc_zeroed_user_highpage_movable(vma, address);
	}
	...
	if ((vma->vm_flags & VM_POS)||(POS_AREA_START <= address && address < POS_AREA_END))
	{
	//	printk("[POSDEBUG]before add rmap\t%s\tpfn\t%ld\tmap_count\t%d\n",__func__,page_to_pfn(page),atomic_read(&page->_mapcount));
		if(atomic_read(&page->_mapcount) == -1)
		{
			//printk("[POSDEBUG] First rmap\t%s\tpfn\t%ld\tmap_count\t%d\n",__func__,page_to_pfn(page),atomic_read(&page->_mapcount));
			page_add_new_anon_rmap(page,vma,address);
		}
		else
		{
			//printk("[POSDEBUG] Normal rmap\t%s\tpfn\t%ld\tmap_count\t%d\n",__func__,page_to_pfn(page),atomic_read(&page->_mapcount));
			do_page_add_anon_rmap(page,vma,address,exclusive);
		}
	//	printk("[POSDEBUG]after add rmap\t%s\tpfn\t%ld\tmap_count\t%d\n",__func__,page_to_pfn(page),atomic_read(&page->_mapcount));
	}
	else
		page_add_new_anon_rmap(page, vma, address);
setpte:
