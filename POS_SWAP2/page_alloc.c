...
static int
__pos_perform_reclaim(gfp_t gfp_mask, unsigned int order, struct zone* zone, int mode)
{
	...
	progress = pos_try_to_free_pages(zone, order, gfp_mask, mode);
	//printk("{POS DEBUG} %s is call and progress is %d\n", __func__, progress);
	//dump_stack();
...
pos_get_page_from_freelist(struct zone* zone, gfp_t gfp_mask, unsigned int order,
		int alloc_flags, int migratetype, int mode)
{
	struct page *page = NULL;
	int classzone_idx;
	unsigned long mark;
	...
	*/
	...
	//	printk("{POS DEBUG} %s is called and", __func__);
		/*
		if (zone_reclaim_mode == 0 ||  !zone_allows_reclaim(zone, zone))
		{
			printk(" __pos_perform call fail\n");
			goto stop_try_this_zone;
		}
		*/
		/*
		...
		//ret = zone_reclaim(zone, gfp_mask, order);
		///////////////////////////
		//printk(" __pos_perform call Success\n");
		ret = __pos_perform_reclaim(gfp_mask, order, zone, mode);
		//////////////////////////
	...
	int alloc_flags, int migratetype, unsigned long *did_some_progress)
{
	...
	*did_some_progress = __pos_perform_reclaim(gfp_mask, order, zone, 1);
...
rebalance:
	...
	page = pos_get_page_from_freelist(zone, gfp_mask, order, 
			alloc_flags & ~ALLOC_NO_WATERMARKS, migratetype, 0);
	if (page)
		goto got_pg;
	...
	if (!wait) {
		 ...
		 */
		//printk("[POS DEBUG] %s What the wait?\n",__func__);
	...
	{
		//printk("[POS DEBUG] %s PF_MEMALLOC\n",__func__);
		goto nopage;
	}
	/* Avoid allocations with no watermarks from looping endlessly */
	if (test_thread_flag(TIF_MEMDIE) && !(gfp_mask & __GFP_NOFAIL))
	{	
		//printk("[POS DEBUG] %s test_thread_flag\n",__func__);
		goto nopage;
	}
	...
	if (!did_some_progress) {
		//printk("[POS DEBUG] %s did_some_progess\n",__func__);
		goto nopage;
	}
...
//		printk("[POSDEBUG1]%s\tpfn\t%lu\taddr\t%lu\n",__func__,page_to_pfn(page),addr);
//		return page;
        }
...
{
	printk(KERN_ALERT
	       "page:%p [POS]pfn:%ld count:%d mapcount:%d mapping:%p index:%#lx\n",
