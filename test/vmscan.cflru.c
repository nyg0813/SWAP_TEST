...
/*
	...
	*/
                ...
                trace_mm_vmscan_writepage(page, trace_reclaim_flags(page));
                inc_zone_page_state(page, NR_VMSCAN_WRITE);
		printk("[POS PAGEOUT]\tdirect wirte\t%p\t%d\n",page,cycle);
                return PAGE_SUCCESS;
        }
				      ...
				      bool force_reclaim)
{
	...
	cond_resched();
//NYG POS TEMP
	unsigned long nr_iter = 0;
	unsigned long return_check = 0;
//
	...
	while (!list_empty(page_list)) {
		...
		printk("[POS SHRPAGE]\tSHRINK page\t%p\t%d's\t%d\n",page,cycle,nr_iter);
		if (!trylock_page(page))
			goto keep;
		...
		if (PageWriteback(page)) {
		printk("[POS WRITEBA]\tWBs page\t%p\t%d\n",page,cycle);
			/* Case 1 above */
			if (PageReclaim(page) && zone_is_reclaim_writeback(zone)) {
				nr_immediate++;
				goto keep_locked;
		...
		nr_reclaimed++;
// NYG POS TEMP
		printk("[POS RETSUCC]\tREC page\t%p\t%d\n",page,cycle);
//
		list_add(&page->lru, &free_pages);
		continue;
...
keep:
		return_check++;
// NYG POS TEMP
		printk("[POS RETFAIL]\tRTS page\t%p\t%d\n",page,cycle);
//
		list_add(&page->lru, &ret_pages);
		VM_BUG_ON_PAGE(PageLRU(page) || PageUnevictable(page), page);
	}
			       ...
			       enum lru_list lru)
{
	...
	while (!list_empty(&l_hold)) {
		...
		{
			printk("[POS ACTLIST]\tdirty page\t%p\t%d\n",page,cycle);
		...
		{
			if(page_mapped(page)){
				printk("[POS ACTLIST]\tclean page\t%p\t%d mapped\n",page,cycle);
				list_add(&page->lru, &l_inactive);
			}
			else{
				printk("[POS ACTLIST]\tclean page\t%p\t%d unmapped\n",page,cycle);
...
// NYG POS TEMP
		if(total_scanned > 131072&&sc->nr_reclaimed>32)
			goto out;
//
